...
for sub_dir in $sub_dirs; do
    ...
    for sub_path in $sub_paths; do
        ...
        outdir="$(readlink -f output)/${sub_dir_name}/${sub_path}" # output is a symlink to a subdir under the results dir
        log_dir="${outdir}/logs"
        mkdir -p "$log_dir"
...
module load r/3.3.0
set -x
find "${project_parent_dir}/${sub_dir}/pipeline/peaks/results" -path "*${sub_path}*" -type f -name "peaks.bed" | xargs ./peak-summary.R --id-dirname --out-dir "$outdir"
E0F
